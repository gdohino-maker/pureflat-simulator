...
2025-04-01,2284598,20624226,7363311,15768861,34267270,16225448,13684083,1800000,42000000,14000000,12000000,4000000,55000000,15000000`;
  ...
  const runMonteCarloSimulation = (aiSeasonality) => {
    ...
    const targetSales = getTargetSales(currentSales); // ここはドリフト計算用のベース
  ...
  "seasonality": {
    ...
    if (aiResult) {
      try {
        ...
        // シミュレーション結果の最終月（12ヶ月目）の予測中央値を取得し、それを目標のベースにする
        ...
        const combinedResult = {
          ...
          kpiTargets: {
            ...
            targetTraffic: `約 ${Math.round(currentTraffic * Math.sqrt(actualMultiple)).toLocaleString()} / 月`, // 実際の倍率を使って計算
            currentCVR: "1.2 %",
